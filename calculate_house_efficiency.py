...
def fetch_degree_days_data():

    # read file with degree days for Reading
    degree_days_2022 = pd.read_csv(
        "EGUB_HDD_15.5C.csv", parse_dates=["Date"], dayfirst=True
    )

    return degree_days_2022
...
heating_settings = {"hours": hours_heating, "temp": room_temp}

year_bill_well = 0
year_energy_well = 0

degree_days_data = fetch_degree_days_data()

# find minimum date from available data
min_date = datetime.datetime(
    year=degree_days_data["Date"].min().year,
    month=degree_days_data["Date"].min().month,
    day=degree_days_data["Date"].min().day,
)
# find maximum date from available data
max_date = datetime.datetime(
    year=degree_days_data["Date"].max().year,
    month=degree_days_data["Date"].max().month,
    day=degree_days_data["Date"].max().day,
)
if datetime.datetime.today() > max_date:
    # streamlit bug!! current day must be in the interval
...
date_start = st.date_input(
    "Insert meter reading date (start)",
    min_value=min_date,
    max_value=max_date,
)
...
date_end = st.date_input(
    "Insert meter reading date (end)",
    min_value=min_date,
    max_value=max_date,
)

meter_readings = {
    "first": {"date": str(date_start), "reading": reading_start},
    "second": {"date": str(date_end), "reading": reading_end},
}


def calculate_efficiency_score(
    heating_settings, meter_readings, property_info, degree_days_data
):

    # check that the meter readings are valid
    actual_energy = (
        meter_readings["second"]["reading"] - meter_readings["first"]["reading"]
    )
    ...
    df = degree_days_data[
        (
            (degree_days_data["Date"].ge(meter_readings["first"]["date"]))
            & (degree_days_data["Date"].le(meter_readings["second"]["date"]))
        )
    ].copy()
    ...
    for _, row in df.iterrows():
        ...
        energy_day_well = calculate_energy_consumption.main(
            property_info, temp_gap, month, heating_settings["hours"], "WELL_INSULATED"
        )
        energy_day_poor = calculate_energy_consumption.main(
            property_info,
            temp_gap,
            month,
            heating_settings["hours"],
            "POORLY_INSULATED",
        )
    ...
    score = round(
        ...
        * (
            1
            - (actual_energy - total_energy_well)
            / (total_energy_poor - total_energy_well)
        )
    )
...
efficiency = calculate_efficiency_score(
    ...
    degree_days_data=degree_days_data,
)
