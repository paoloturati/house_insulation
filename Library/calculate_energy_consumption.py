...
def main(property_info, temp_gap, month, hours_day, insulation):
    ...
    if property_info["walls_height"] is np.nan:
        ...
        volume_property = property_info["area_floor"] * property_info["floors"] * PARAMS_PROPERTY["WALLS_HEIGHT"]
    else:
        volume_property = property_info["area_floor"] * property_info["floors"] * property_info["walls_height"]
    # calculate energy lost through floor
    energy_floor = property_info["area_floor"] * temp_gap * PARAMS_PROPERTY[insulation]["floor"]
    # calculate energy lost through roof
    energy_roof = property_info["area_roof"] * temp_gap * PARAMS_PROPERTY[insulation]["roof"]
    # calculate energy lost through windows
    energy_windows = property_info["area_windows"] * temp_gap * PARAMS_PROPERTY[insulation]["windows"]
    # calculate energy lost through walls
    energy_walls = property_info["area_walls"] * temp_gap * PARAMS_PROPERTY[insulation]["walls"]
    # calculate energy lost through drafts
    energy_drafts = PARAMS_PROPERTY[insulation]["ACH"] * PARAMS_ENVIRONMENT["SEASON_ACH"][month] * volume_property * temp_gap
    # add up all components
    total_energy_hour = energy_floor + energy_roof + energy_windows + energy_walls + energy_drafts
